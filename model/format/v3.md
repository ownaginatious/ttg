...
A delta serialization format was not yet defined and implemented at the time of writing this document, so a hypothetical example will be instead for explanatory purposes.
...
```json
{
  ...
  "value_deltas": {
    "additions": [<add_delta>, ...],
    "removals": [<remove_delta>, ...],
    "replacements": [<replace_delta>, ...],
  },
  "substructure_deltas": [<structure_delta>, ...]
}
```
...
_Note: Value deltas are represented as a single array in the structure delta model._

### AddDelta

An `AddDelta` represents a removal of a substructure or attribute (i.e. setting to null or deletion from a set/array)
